...
class Sales extends Base_Controller {
	...
	function sales_quotation($action='',$id='')
	{
		...
		// $data['terms_conditions']=$this->Sales_model->getTermsConditions('','payment');
		...
		if($action=='delete')
		{
			...
			$details=$this->Sales_model->deleteSalesQuotation($data['id']);
			if($details)
			{
	...
	function create_tc($action='',$enc_id='')
	{

		$data['title'] = "Create Terms & Conditions"; 
		...
		if($this->input->post('submit') )
		{
			...
			$this->form_validation->set_rules('tc_type', 'Type', 'required|in_list[payment,normal]');
			if( $this->form_validation->run() )
			{
				$post_arr=$this->input->post();
				$this->Sales_model->begin();
				$id=$this->Sales_model->insertTermsConditions($post_arr);
				if ($id) {
					$this->Sales_model->commit();
					$this->Base_model->insertIntoActivityHistory(log_user_id(), log_user_id(),'Terms And Conditions Added', serialize($post_arr));

					$enc_id=$this->Base_model->encrypt_decrypt('encrypt',$id);
					$this->redirect('Sales Quotation Added Successfully',"sales/create-tc",TRUE);
				}
				else{
					$this->Sales_model->rollback();
					$this->redirect("Adding Failed","sales/create-tc",FALSE);
				}
			}
		}
		...
		if($enc_id){
			...
			$data['details'] = $this->Sales_model->getAllTC($id);
			
		}else{
			$data['details'] = $this->Sales_model->getAllTC();
		}
		...
		if($this->input->post('update') )
		{
			...
			if( $this->form_validation->run() )
			{
				...
				$this->Sales_model->begin(); 
				$post_arr['id'] = $id;
				$id=$this->Sales_model->updateTermsConditions($post_arr);
				if ($id) {
					$this->Sales_model->commit();
					$this->Base_model->insertIntoActivityHistory(log_user_id(), log_user_id(),'Terms And Conditions udpated', serialize($post_arr));

					$enc_id=$this->Base_model->encrypt_decrypt('encrypt',$id);
					$this->redirect('Sales Quotation Updated Successfully',"sales/create-tc",TRUE);
				}
				else{
					$this->Sales_model->rollback();
					$this->redirect("Updating Failed","sales/create-tc/edit/{$enc_id}",FALSE);
				}
			}
		}
	...
	function add_items($enc_id='',$action='',$id='')
	{
		...
		// $data['code']=$this->Item_model->getMaxItemId()+1001;
	...
	function edit_sales($id)
	{
		...
		if($this->input->post('update') && $this->validate_edit_sales())
		{
			$post_arr=$this->input->post();
		...
		if ($this->input->post('id')) {
			...
			if( $this->form_validation->run() ){
				...
				if ($response['item_info']) {
					// if ($quantity > $response['item_info']['total_quantity']) {
					// 	$response['success'] = FALSE;
					// 	$response['msg'] = "Quantity Not Available";
	...
	function validate_edit_sales() 
	{
		...
		$this->form_validation->set_rules('normal_terms_id', 'Normal T&C', 'trim|required');
...
// print_r($post_arr); die();
	...
	public function get_sales_list_ajax() {
		if ($this->input->is_ajax_request()) {
			...
			$details = $this->Sales_model->getSalesDetailAjax( $post_arr,'');
			// print_r($details);die();
			$response = array(
				"draw" => intval($draw),
				"iTotalRecords" => $count_without_filter,
...
// die();
			...
			$response['success'] = TRUE; 
			$response['html'] = 
			$this->smarty->view("admin/sales/edit_items.tpl", $data, TRUE);
			$this->output
			->set_status_header(200)
			->set_content_type('application/json', 'utf-8')
			->set_output(json_encode($response, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES))
