...
<style type="text/css">
    ...
    <div class="row "> 
        <div class="col-sm-12 hidden-print"> 
            <div class="text-right">
                ...
                <a class="btn btn-primary pull-right"  href="{base_url()}salesman/showroom/create">
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <select id="meeting_id" name="meeting_id" class="meeting_code_ajax form-control" >
                                            {if $post_arr['meeting_id']}
                                            <option value="{$post_arr['meeting_id']}">{$post_arr['code']}</option>
                                            {/if} 
                                        </select> 
                                    </div>
                                </div> 
                                ...
                                <div class="col-md-2" style="max-width: 237px;">
                                    <div class="form-group">
                                        ...
                                        <select class="selectpicker" data-size="7" data-style="select-with-transition" title="active" name="status" id="status" > 
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header card-header-rose card-header-icon">
                    ...
                    <h4 class="card-title">Available Meeting Minutes</h4>
                </div>
                <div class="card-body"> 
    ...
    <script>
        ...
        $('.meeting_code_ajax').select2({

            placeholder: 'Meeting Minutes',
            ajax: {
                url:'{base_url()}salesman/autocomplete/meeting_Auto_ajax',
        ...
        $('body').on('click', '.edit-sales', function(){
            ...
            function (isConfirm) {
                if (isConfirm) {
                    document.location.href = '{base_url()}' + "salesman/showroom/edit-meeting/"+id; 
                } else {
        ...
        $('body').on('click', '.delete-sales', function(){
            ...
            function (isConfirm) {
                if (isConfirm) {
                    document.location.href = '{base_url()}' +"salesman/showroom/create/delete/"+id; 
                } else {
                    swal('{lang('text_cancelled')}','{lang('your_content_safe')}', "error");
                }
            });
        ...
        $('body').on('click', '.approve-sales', function(){
            ...
            function (isConfirm) {
                if (isConfirm) {
                    document.location.href = '{base_url()}' + "salesman/sales/approve_sales/"+id; 
                } else {
                    swal('{lang('text_cancelled')}','{lang('your_content_safe')}', "error");
                }
            });
    ...
    var order = $('#sales_list').DataTable({
        ...
        'ajax': {
            'url':'{base_url()}salesman/showroom/get_meeting_list_ajax',
            'type': "POST", 
            'data' : {
       ...
       { data: 'customer_name',mRender: function(data, type, row) {
        ...
        { data: 'id', mRender: function(data, type, row) {
            ...
            if(row.status != 'deleted'){
               ...
               link +="<a href='{base_url()}login/showroom-items-print/"+row.enc_id+"'  rel='tooltip' title='Print Out' data-id='"+row.enc_id+"'class='print_out'><i class='material-icons fa fa-print'></i></a>";
