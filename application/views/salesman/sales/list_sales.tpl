...
<style type="text/css">
	...
	<script>
		...
		$('.sales_code_ajax').select2({
			...
			ajax: {
				url:'{base_url()}salesman/autocomplete/sales_code_ajax',
		...
		$('body').on('click', '.edit-sales', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url()}' + "salesman/sales/edit-sales/"+id; 
				} else {
		...
		$('body').on('click', '.delete-sales', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url()}' +"salesman/sales/sales-quotation/delete/"+id; 
				} else {
					swal('{lang('text_cancelled')}','{lang('your_content_safe')}', "error");
				}
			});
		...
		var order = $('#sales_list').DataTable({
			...
			'ajax': {
				'url':'{base_url()}salesman/sales/get_sales_list_ajax',
				'type': "POST", 
				'data' : {
					'type' : '{$post_arr['type']}',
			...
			{ data: 'id', mRender: function(data, type, row) {
				...
				if(row.status != 'deleted'){
					...
					link +="<a href='{base_url()}salesman/sales/print_details/"+row.enc_id+"'  rel='tooltip' title='Print Out' data-id='"+row.enc_id+"'class='print_out'><i class='material-icons'>print</i></a>";
