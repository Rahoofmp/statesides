	...
	<div class="row">
								...
								<div class="row">  
									<div class="col-md-6" style="max-width: 500px;">
										<div class="form-group">
											<select class="selectpicker" data-size="7" data-style="select-with-transition" title="{ucfirst($details['status'])|replace:'_':' '}" name="status" id="status" >

												<option value='send_to_delivery' {set_select('status', 'delivered')}>Send to Delivery</option>
		...
		<script>	
			function printQrCode(ele) {
				...
				$.ajax({
					type:'POST',
					url:"{base_url('admin/delivery/save-location-details')}",
					data: { id: id,  location_details: loc,  } ,
					dataType:'json',
				})
				.done(function( response ) { 

					url  = "{base_url('admin/delivery/delivery-details')}";
					if(response.success){
