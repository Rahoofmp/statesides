...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		var order = $('#example').DataTable({
			...
			'ajax': {
				'url':'{base_url()}admin/delivery/get_delivery_ajax',
				"type": "POST",
				"data" : {
			...
			mRender: function(data, type, row) {
				...
				var link = '<a href = "'+ '{base_url()}' + 'admin/delivery/delivery_details/' + row.enc_id +'" class="" data-placement="top" title ="View Order Details" target="_blank"><i class="material-icons text-info" aria-hidden="true">local_see</i></a>';

				if(row.status=='pending'){
					link += '<a data-id="'+row.enc_id+'" class="edit_delivery_note " id="edit" title="Edit"><i class="material-icons text-rose" aria-hidden="true">edit</i></a>';
					link += '<a data-id="'+row.enc_id+'" class="delete_delivery_note " id="delete" title="Delete"><i class="material-icons text-danger" aria-hidden="true">delete</i></a>';
				}
	...
	$(document).on("click", "body #example tbody .delete_delivery_note" , function() {
			...
			$.confirm({
				...
				buttons: {
					confirm: function () {
						...
						document.location.href = '{base_url(log_user_type())}' +"/delivery/delivery-list/delete/"+id; 
					},
					cancel: function () {
						$.alert('Canceled!');
					},
