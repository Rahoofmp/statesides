				...
				<div class="card-body"> 
					...
					<div class="form-body">
						<div class="row"> 
							...
							<div class="col-md-4">
								<div class="form-group">
							<select name="department" class="deptname_ajax form-control" >
								{if $department_id}
...
{if $job_order_id}
{if $job_details['actual_workin_hrs']>$job_details['estimated_working_hrs']}
<div class="alert alert-rose alert-with-icon" data-notify="container">
	...
	<span data-notify="message">The Spent Time is greater than Estimated Working Time By {$job_details['actual_workin_hrs'] - $job_details['estimated_working_hrs']} Hrs</span>
...
<div class="row">
				...
				<div class="card">
					...
					<div class="card-body ">
						<div class="form-group">
							...
							<input type="text" class="form-control datepicker" value="{$smarty.now|date_format:"%Y-%m-%d"}" name="date">
						...
						{else}
						<input type="hidden" class="form-control" required name="department" value="{$department}">
					{form_error('department')}
						{/if}
						<div class="form-group">
						...
						<div class="table-responsive">
							<table class="table table-hover">
								...
								<tbody> 
									...
									<tr>
										<td>Estimate Time: </td>
										<td> <h6>{$job_details['estimated_working_hrs']}  Hrs</h6></td> 
									</tr>
									<tr>
										<td>Spent Time: </td>
										<td><h6>{$job_details['actual_workin_hrs']} Hrs</h6></td> 
									</tr> 
								</tbody>
							</table>
						</div>
			...
			<div class="col-md-12">
				...
				<div class="card">
					...
					<div class="card-body ">
						...
						<div class="form-group">
							<select  name="department_change_id" class="deptname_ajax form-control" >
								{if $department_id}
								<option value="{$department_id}">{$department_name}</option>
								{/if} 
							</select> 
	...
	<div class="col-md-6">
				...
				<div class="timeline-panel">
					...
					<div class="row"> 
						...
						<div class="btn-actions text-right col-sm-3"> 
							...
							<a rel="tooltip" title="Delete" href="javascript:delete_event('/{$item.enc_id}/{$item.enc_job_id}/{$item.enc_department_id}')" class="btn btn-danger btn-link"><i class="material-icons">delete</i></a> 
						</div>
					</div>
				</div>
			</li>
		...
		{if $key == 5}
		<a href="{base_url()}{log_user_type()}/jobs/day-progress-list/{$job_order_id}/{$department_id}" target="_blank"><i class="material-icons">local_offer</i> {lang('text_view_more')}</a>
		{/if}
...
<script> 
	...
	$('.deptname_ajax').select2({
		placeholder: 'Select Department Name',
		ajax: {
			url:'{base_url(log_user_type())}/autocomplete/department_job_name_ajax',
			type: 'post',
			dataType: 'json',
			delay:250,
			data: function (params) {
				...
				var query = {
					search: params.term,
					job_order_name: $('#job_order_name').val()
				}
				return query;
			},
			processResults: function(data) {
	...
	$(document).ready(function () {
		$('.status').on('change', function () {
			...
			$.ajax({
				type: "POST",
				url: "{base_url(log_user_type())}/project/changeStatus",
