								...
								<div class="row">
									<div class="col-lg-6">
										<div class="input-group form-control-lg">
											...
											<div class="col-sm-10">
												<div class="form-group">
													<label class="bmd-label-floating"> Salesman </label>

													<select id="sales_name" name="sales_name" class="sales_ajax form-control" >
														{if $details['sales_id']}
														<option value="{$details['sales_id']}">{$details['salesman_name']}</option>
														{/if} 

													</select> 
													{form_error("sales_name")} 
												</div>
											</div>
										</div>
									</div>
									...
									<div class="table-responsive">
										<table class="table" style="table-layout: fixed; width: 100%;">
											...
											<tbody>
												...
												{foreach $details['items'] as $key=>$v} 
...
<script type="text/javascript">
	...
	function remove_meeting_sample(id, meeting_id,path)
	{
		swal({
		...
		function (isConfirm) {
			if (isConfirm) {
				document.location.href = '{base_url()}' +"admin/showroom/remove_meeting_sample/"+id+'/'+meeting_id; 
			} else {
	...
	$('.items_update').on('click', function(){
		...
		.done(function( response ) { 
			...
			url  = "{base_url('admin/showroom/meeting_mint')}";
			if(response.success){
...
$('.sales_ajax').select2({
	...
	ajax: {
		url:'{base_url()}admin/autocomplete/sales_ajax',
