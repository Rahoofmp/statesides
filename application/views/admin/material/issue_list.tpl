...
<style type="text/css">
					...
					<div class="card-body"> 
						...
						<div class="form-body">
							...
							<div class="col-md-3">
								...
								<div class="col-md-3">
									<div class="form-group">
										<select id="issue_id" name="issue_id" class="material_issue_ajax form-control" >
	...
	<script>
		...
		$('.material_issue_ajax').select2({

			placeholder: 'Select a Voucher number',
			ajax: {
				url:'{base_url(log_user_type())}/autocomplete/material_issue_ajax',
		...
		$('body').on('click', '.edit-package', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url(log_user_type())}' + "/material/edit-issue/"+id; 
				} else {
		...
		var order = $('#material_issue').DataTable({
			...
			'ajax': {
				'url':'{base_url(log_user_type())}/material/get_material_issue_ajax',
				'type': "POST", 
				'data' : {
		...
		$('body').on('click', '.delete-issue', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url()}' +"admin/material/issue-list/delete/"+id; 
				} else {
					swal('{lang('text_cancelled')}','{lang('your_content_safe')}', "error");
				}
			});
