...
<style type="text/css">
					...
					<div class="card-body"> 
						...
						<div class="form-body">
							<div class="row">
								...
								<div class="col-md-3">
									<div class="form-group">
										<select id="receipt_id" name="receipt_id" class="material_receipt_ajax form-control" >
			...
			<div class="card">
				...
				<div class="card-body"> 
					<div class="table-responsive">
						<table class="table" id="material_receipt">
	...
	<script>
		...
		$('body').on('click', '.edit-package', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url(log_user_type())}' + "/material/edit-receipt/"+id; 
				} else {
		...
		var order = $('#material_receipt').DataTable({
			...
			'ajax': {
				'url':'{base_url(log_user_type())}/material/get_material_list_ajax',
				'type': "POST", 
				'data' : {
		...
		$('body').on('click', '.delete-receipt', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url()}' +"admin/material/list/delete/"+id; 
				} else {
					swal('{lang('text_cancelled')}','{lang('your_content_safe')}', "error");
				}
			});
