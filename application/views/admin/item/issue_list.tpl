...
<style type="text/css">
					...
					<div class="card-body"> 
						...
						<div class="form-body">
							<div class="row">
								...
								<div class="col-md-3">
									<div class="form-group">
										<select id="issue_id" name="issue_id" class="consumable_issue_ajax form-control" >
			...
			<div class="card">
				...
				<div class="card-body"> 
					<div class="table-responsive">
						<table class="table" id="consumable_issue">
	...
	<script>
		...
		$('.consumable_issue_ajax').select2({

			placeholder: 'Select a Voucher number',
			ajax: {
				url:'{base_url(log_user_type())}/autocomplete/consumable_issue_ajax',
		...
		$('body').on('click', '.edit-package', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url(log_user_type())}' + "/item/edit-issue/"+id; 
				} else {
		...
		var order = $('#consumable_issue').DataTable({
			...
			'ajax': {
				'url':'{base_url(log_user_type())}/item/get_material_issue_ajax',
				'type': "POST", 
				'data' : {
			...
			'columns': [
				...
				{ data: 'id', mRender: function(data, type, row) {
					...
					if(row.status != 'deleted'){
						...
						link +="<a href='{base_url()}admin/item/print_history/"+row.enc_id+"'  rel='tooltip' title='Print Out' data-id='"+row.enc_id+"'class='print_out'><i class='material-icons fa fa-print'></i></a>";
					}else{	
						link +='Deleted';
					}
		...
		$('body').on('click', '.delete-issue', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url()}' +"admin/item/issue-list/delete/"+id; 
				} else {
					swal('{lang('text_cancelled')}','{lang('your_content_safe')}', "error");
				}
			});
