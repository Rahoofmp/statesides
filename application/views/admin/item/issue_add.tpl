			...
			<div class="card-body  mt-3">
					...
					<div class="input-group form-control-lg">
						...
						<div class="col-sm-9">
							<div class="form-group">
								<label for="voucher_number" class="bmd-label-floating">Issue Voucher No </label>
				...
				<div class="col-sm-12">
					<div class="input-group form-control-lg">
						...
						<div class="col-sm-9">
							<div class="form-group">
								<label for="voucher_date" class="bmd-label-floating">Issue Date </label>
				...
				<div class="col-sm-12">
					<div class="input-group form-control-lg">
						<div class="input-group-prepend">
							<span class="input-group-text"><i class="material-icons">account_box</i>
							</span>
						</div>
						<div class="col-sm-9">
							<div class="form-group">
								<label for="requested_by" class="bmd-label-floating">Requested By</label>
								{* <input type="text" class="form-control" id="requested_by" name="requested_by" required>  *}
								<select id="requested_by" name="requested_by" class="requested_by form-control" >
									{if $smarty.post.requested_by}
									<option value="{$smarty.post.requested_by}">{$requested_name}</option>
									{/if}
								</select>	
								{form_error('requested_by')}
							</div>
						</div>
					</div>
				</div>
				<div class="col-sm-12">
					<div class="input-group form-control-lg">
						...
						<div class="col-sm-9">
							<div class="form-group">
								<label for="issued_by" class="bmd-label-floating">Issued By</label>
								<input type="text" class="form-control" id="issued_by" name="issued_by" required value="{log_user_name()}" readonly> 
								{form_error('issued_by')}
							</div>
						</div>
					</div>
				</div>
					...
					<div class="input-group form-control-lg">
						...
						<div class="col-sm-9">
							<div class="form-group">
								...
								<select id="received_by" name="received_by" class="received_by form-control" >
									{if $smarty.post.received_by}
									<option value="{$smarty.post.received_by}">{$employee_name}</option>
									{/if}
								</select>					

								{form_error('received_by')}
							</div>
						</div>
					</div>
				</div>
...
<script>
	...
	$(document).ready(function(){ 
		...
		$('.requested_by').select2({
			...
			ajax: {
				url:'{base_url()}admin/autocomplete/employee_ajax',
		...
		function addRow () {
			...
			if(validFrom == true){
				...
				$.ajax({
					type:'POST',
					url:"{base_url(log_user_type())}/item/check-consumable-items",
					data: { 
		...
		$('.items_update').on('click', function(){
			...
			$.ajax({
				type:'POST',
				url:"{base_url('admin/item/save-issue-items')}",
				data: { 
			...
			.done(function( response ) {
				url  = '{base_url()}'+'admin/item/issue-list';
				if(response.success){
