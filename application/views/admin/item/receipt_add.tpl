		...
		<div class="card ">
					...
					<div class="input-group form-control-lg">
						...
						<div class="col-sm-9">
							<div class="form-group">
								...
								<input type="text" class="form-control" id="bill_number" name="bill_number" required value="{set_value('bill_number')}"> 
			...
			{* 	<div class="col-sm-12">
					<div class="input-group form-control-lg">
						...
						<div class="col-sm-9">
							<div class="form-group">
								<label for="employee_name" class="bmd-label-floating">Employee Name </label>
						...
						<div class="col-sm-9">
							<div class="form-group">
								<select id="employee_id" name="employee_id" class="employee_id form-control" >
									...
									<option value="{$smarty.post.employee_id}">{$employee_name}</option>
									{/if}
								</select>					
					...
					<div class="input-group form-control-lg">
						...
						<div class="form-group col-sm-9">
							<select id="supplier_id" name="supplier_id" class="supplier_id form-control" >
								{if $smarty.post.supplier_id}
								<option value="{$smarty.post.supplier_id}">{$supplier_name}</option>
								{/if}
							</select>	
	...
	$(document).ready(function(){ 
		...
		function addRow () {
			...
			if(validFrom == true){
				...
				$.ajax({
					type:'POST',
					url:"{base_url(log_user_type())}/item/check-items",
					data: { 
				...
				.done(function( response ) { 
					...
					if(response.status && response.data){
						...
						const items_arr = response.items_arr;
						console.log(data)
						...
						$('.item_ajax').val('').trigger('change'); 
						$('.job_orderid_ajax').val(null).trigger('change');
						$('.project_ajax').val(null).trigger('change').select2('open');
		...
		$('.items_update').on('click', function(){
			...
			$.ajax({
				type:'POST',
				url:"{base_url('admin/item/save-items1')}",
				data: { 
			...
			.done(function( response ) {
				url  = '{base_url()}'+'admin/item/receipt-list';
				if(response.success){
