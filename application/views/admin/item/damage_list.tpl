...
<style type="text/css">
					...
					<div class="card-body"> 
						...
						<div class="form-body">
							...
							{* 	<div class="col-md-3">
								...
								<div class="col-md-3">
									<div class="form-group">
										<select id="issue_id" name="issue_id" class="consumable_damage_ajax form-control" >
	...
	<script>
		...
		$('.consumable_damage_ajax').select2({

			placeholder: 'Select a Voucher number',
			ajax: {
				url:'{base_url(log_user_type())}/autocomplete/consumable_damage_ajax',
		...
		$('body').on('click', '.edit-package', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url(log_user_type())}' + "/item/edit-damage/"+id; 
				} else {
		...
		var order = $('#material_issue').DataTable({
			...
			'ajax': {
				'url':'{base_url(log_user_type())}/item/get_material_damage_ajax',
				'type': "POST", 
				'data' : {
			...
			'columns': [
				...
				{ data: 'id', mRender: function(data, type, row) {
					...
					if(row.status != 'deleted'){
						...
						link +="<a href='{base_url()}admin/item/damage_history/"+row.enc_id+"'  rel='tooltip' title='Print Out' data-id='"+row.enc_id+"'class='print_out'><i class='material-icons fa fa-print'></i></a>";
					}else{	
						link +='Deleted';
					}
		...
		$('body').on('click', '.delete-issue', function(){
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = '{base_url()}' +"admin/item/damage-list/delete/"+id; 
				} else {
					swal('{lang('text_cancelled')}','{lang('your_content_safe')}', "error");
				}
			});
