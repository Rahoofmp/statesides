						...
						<div class="input-group form-control-lg">
							...
							<div class="col-sm-12">
								<div class="input-group form-control-lg">
									...
									<div class="col-sm-9">
										<div class="form-group">
											<label for="issued_by" class="bmd-label-floating">Return By</label>
											<select id="return_by" name="return_by" class="return_by form-control" >
												{if $material_data['return_by']}
												<option value="{$material_data['return_by']}">{$material_data['return_name']}</option>
												{/if}
											</select>					
									...
									<div class="col-sm-9">
										<div class="form-group">
											...
											<select id="received_by" name="received_by" class="received_by form-control" >
												{if $material_data['received_by']}
												<option value="{$material_data['received_by']}">{$material_data['receiver_name']}</option>
												{/if}
											</select>					
	...
	<script>
		...
		// });
			...
			$('.items_update').on('click', function(){
				...
				$.ajax({
					type:'POST',
					url:"{base_url('admin/item/update-return-items')}",
					data: { 
...
function remove_material_receipt(material_item_id, issue_id)
{
	...
	$.confirm({
		...
		buttons: {
			confirm: function () {
				document.location.href = '{base_url()}' + "admin/item/remove_return_item/"+material_item_id+'/'+issue_id;
			},
			cancel: function () {
				$.alert('Canceled!');
			},
