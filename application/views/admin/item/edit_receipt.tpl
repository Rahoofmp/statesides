						...
						<div class="col-sm-9">
							<div class="form-group">
								<select id="employee_id" name="employee_id" class="employee_id form-control" >
									<option value="{$details['name']}">{$details['employee_name']}</option> 
								</select>	
								<input type="hidden" class="form-control" id="receipt_id" name="receipt_id"  value="{$receipt_id}">					
...
<script type="text/javascript">
	...
	$(document).ready(function(){ 
		...
		$('.items_update').on('click', function(){
			...
			$.ajax({
				type:'POST',
				url:"{base_url('admin/item/edit-save-items')}",
				data: { 
...
function remove_material_item(material_item_id, reciept_id)
{
	...
	$.confirm({
		...
		buttons: {
			confirm: function () {
				document.location.href = '{base_url()}' + "admin/item/remove-material-item/"+material_item_id+'/'+reciept_id;
			},
			cancel: function () {
				$.alert('Canceled!');
			},
