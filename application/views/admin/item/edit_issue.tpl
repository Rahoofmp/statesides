						...
						<div class="input-group form-control-lg">
							...
							<div class="col-sm-9">
								<div class="form-group">
									...
									<input type="text" class="form-control" id="requested_by" name="requested_by" required value="{$material_data['requested_by']}"> 
						...
						<div class="input-group form-control-lg">
							...
							<div class="col-sm-9">
								<div class="form-group">
									...
									<label for="received_by" class="bmd-label-floating">Received By</label>
									<select id="received_by" name="received_by" class="received_by form-control" >
										{if $material_data['receiver_name']}
										<option value="{$material_data['receiver_name']}">{$material_data['receiver_name']}</option>
										{/if}
									</select>					
		...
		// });
			...
			function addRow () {
				...
				if(validFrom == true){
					...
					$.ajax({
						type:'POST',
						url:"{base_url(log_user_type())}/item/check_consumable_items",
						data: { 
			...
			$('.items_update').on('click', function(){
				...
				$.ajax({
					type:'POST',
					url:"{base_url('admin/item/update-issue-items')}",
					data: { 
...
function remove_material_receipt(material_item_id, issue_id)
{
	...
	$.confirm({
		...
		buttons: {
			confirm: function () {
				document.location.href = '{base_url()}' + "admin/item/remove_issue_item/"+material_item_id+'/'+issue_id;
			},
			cancel: function () {
				$.alert('Canceled!');
			},
