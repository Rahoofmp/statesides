						...
						<div class="row">
							...
							<div class="col-sm-4">
								<div class="form-group">
									<input type="text" name="name" id="name" {if $id} value="{$vat_detail['name']}"{else}placeholder="Name" {/if}class=" form-control col-12">
									<input type="text" name="id" value="{$vat_detail['id']}" hidden="">
									{form_error('name')}
								</div>
							</div>
							<div class="col-sm-4">
								<div class="form-group">
									<input type="text" name="value" id="value"{if $id} value="{$vat_detail['value']}" {else} placeholder="Value(%)" {/if} class=" form-control col-12">
									{form_error('value')}
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	function edit_vat(id)
	{
		...
		function (isConfirm) {
			if (isConfirm) {
				document.location.href = '{base_url()}' + "admin/member/create_vat/"+id; 
			} else {
	...
	function delete_department(id)
	{
		...
		function (isConfirm) {
			if (isConfirm) {
				document.location.href = '{base_url()}' +"admin/member/create_vat/"+id+"/delete"; 
			} else {
				swal('{lang('text_cancelled')}','{lang('your_content_safe')}', "error");
			}
		});
