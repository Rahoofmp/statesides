...
var ModuleConfig = function() {
    ...
    var runSwitchChange = function() { 
        $("input[type='checkbox'].module-update").on('switchChange.bootstrapSwitch', function(event, state)
        { 
            var newState = (state == true) ? 'On' : 'Off' ;
            var title = $(this).data("lang");  
            var infoBox = 'success';  
            var Url = $("#rootPath").val() + "admin/settings/moduleUpdate";  
            $.post(Url,{
                module_name: this.id,
                status: state
            },function(data){ 
        ...
        $("input[type='checkbox'].sub-module-update").on('switchChange.bootstrapSwitch', function(event, state)
        {  
            var newState = (state == true) ? 'On' : 'Off' ;
            var title = $(this).data("lang");
            var infoBox = 'success';  
            var Url = $("#rootPath").val() + "admin/settings/subModuleUpdate";  
            $.post(Url,{
                sub_module_type: this.id,
                sub_module_name: $(this).data("sub-module"),
                status: state
            },function(data){ 
                if(data){
                    if(state == true){
        ...
        $("input[type='checkbox'].language-update").on('switchChange.bootstrapSwitch', function(event, state)
        {  
            var newState = (state == true) ? 'On' : 'Off' ;
            var title = $(this).data("language-name");
            var language_id = this.id;
            var language_type = $(this).data("language-type");
            var infoBox = 'success';  
            var Url = $("#rootPath").val() + "admin/settings/languageUpdate";  
            ...
            $.post(Url, {
                ...
                if(data.status){
                    ...
                    toastr[infoBox]('Updated to <b>' + newState + '</b> Succesfully', title);
                }else{

                    $('#'+language_id+'.'+language_type).bootstrapSwitch('toggleState', true, true);
